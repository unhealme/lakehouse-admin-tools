...
func AutoKillApps(logger *internal.Logger, yarnClient *yarn.YarnRMClient, args *cmd.YarnAutoKillAppsArgs) {
	logger.Debug("using auto kill apps args.", logger.Args(internal.ToArgs(*args)...))
	apps, err := yarnClient.Applications(logger, []yarn.ApplicationState{yarn.RUNNING}, "", "", 0)
	if err != nil {
	...
	if total > 0 {
		...
		for _, app := range appToKill {
			ok := true
			if !args.DryRun {
				ok = yarnClient.KillApplication(logger, app)
			}
			if ok {
