...
release:
	GOOS=windows go build -a -trimpath -ldflags="-s -w -extldflags=-static" -o build/windows/$(prog)
	GOOS=linux go build -a -trimpath -ldflags="-s -w -extldflags=-static" -o build/linux/$(prog)
	GOAMD64=v3 GOOS=windows go build -a -trimpath -ldflags="-s -w -extldflags=-static" -o build/windows/$(prog)_amd64v3
